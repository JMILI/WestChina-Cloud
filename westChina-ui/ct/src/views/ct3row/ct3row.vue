          ...
          <el-collapse-item title="病人study列表" name="2" class="left-study">
            ...
            >
              ...
              <el-collapse-item :title="'studyID:'+key" class="left-study-collapse left-study-collapse-item" name="3">
                ...
                >
                  <div
                    :id="studySeriesList[key][childrenKey].dicomId"
                    class="ct-image1"
...
export default {
  ...
  mounted() {
    ...
    that.initListCanvas()
    that.initTwoCanvas()
    // console.log("-------------1----------",cornerstone.getEnabledElements())
    ...
    element1.addEventListener(cornerstoneTools.EVENTS.MEASUREMENT_COMPLETED, function (params) {
      ...
      // console.log("dangqian",that.canvasStack1.currentImageIdIndex)
      // console.log("-----------2------------",cornerstone.getEnabledElements())
      ...
      let canvasTemp = element1.firstElementChild
      myCanvas = that.cropCanvas(
        canvasTemp,
        Math.round(canvasTemp.width / 2 - cols / 2),
        Math.round(canvasTemp.height / 2 - rows / 2),
        cols, rows)
      console.log(that.isImage(myCanvas));
      console.log(that.isCanvas(myCanvas));
      let image = that.convertCanvasToImage(myCanvas)
      console.log(image)
      let files = new FormData();
      files.append("dicom.png", image);
      for (let i = 0; i < files.entries().length; i++) {
        console.log(files.entries()[i])
      }
  ...
  methods: {
    ...
    initListCanvas() {
      ...
      for (const temp in that.studyCanvasList) {
        ...
        // tempCanvas.firstElementChild.style.width='20vw'
        // tempCanvas.firstElementChild.style.height='35vh'
        // console.log(tempCanvas.firstElementChild)
        cornerstone.resize(tempCanvas, false)
        // cornerstone.setCanvasSize(tempCanvas,tempCanvas.firstElementChild)
