...
export default {
  ...
  data() {
    return {
      ...
      imageIds: [
        'http://westChinaBackend:9000/jmtest/test.png',
        'http://westChinaBackend:9000/jmtest/test2.png',
        'http://westChinaBackend:9000/jmtest/test3.png',
        'http://westChinaBackend:9000/jmtest/test4.png',
        // 'wadouri:http://westChinaBackend:9000/dicom/202/6_5.dcm',
        // 'wadouri:http://westChinaBackend:9000/dicom/202/7_6.dcm',
        // 'wadouri:http://westChinaBackend:9000/dicom/202/8_7.dcm',
        // 'wadouri:http://westChinaBackend:9000/dicom/202/9_8.dcm',
      ],
      imageIds2: [
  ...
  methods: {
    ...
    saveImages() {
      let that = this
      let tempSize = that.canvasStack.imageIds.length
      const canvas = document.getElementById('dicomImage1')
      const viewport = cornerstone.getViewport(canvas)
      debugger
      for (let i = 0; i < 1; i++) {
        cornerstone.loadAndCacheImage(that.canvasStack.imageIds[i]).then(function (image) {
          debugger
          ...
          // const bb = cornerstone.getEnabledElementsByImageId(that.canvasStack.imageIds[i])
    ...
    displayCanvas1() {
      ...
      //TODO file
      cornerstone.loadAndCacheImage(that.canvasStack.imageIds[tempIndex])
        .then(function (image) {
          //设置视口
          let viewport = {}
          ...
          cornerstoneTools.addToolState(canvas1, 'stack', that.canvasStack)
        })
  ...
  mounted() {
    const that = this
    this.canvasStack.imageIds = this.imageIds
...
//     );
  ...
  computed: {
     ...
     */
    isCollapse() {
      this.sideBarIsOpen = this.$store.getters.sidebar.opened
